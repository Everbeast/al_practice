...
**enum hack**的行为比较像#define而不像const。
取一个const的地址时合法的
取一个enum的地址就不合法
取一个#define的地址通常也不合法
...
如果抽离出来单独成为一个成员函数还是有重复。如果希望去重，可以使用“运用const成员函数实现出其non-const孪生兄弟”的技术：
...
**在子类构造期间，先构造的是基类部分，而该部分的virtual函数绝不会下降到派生类阶层。即派生类对象的基类构造期间，对象的类型是基类而不是派生类；除此之外，若使用运行期类型信息**（如dynamic_cast和typeid），也会把对象视为基类类型（这样对待是合理的：因为子类部分尚未初始化，如果调用的是子类的虚函数，通常会访问子类部分的数据，会引发安全问题）
...
* **copy赋值运算符**
    ...
    - **继承中**：在派生类的copy赋值运算符中，不要忘记调用基类的copy赋值运算符，否则基类部分会保持不变
      ...
      注：令copy assignment调用copy构造函数不合理：构造已存在的对象
...
若f是程序库内的inline函数，客户将f编进其程序中，一旦程序设计者改变f所有用到f的客户端程序都必须重新编译，若是non inline，客户端只需重连。
...
每个人物(类)包含一个**计算健康值的函数指针**，每创建一个人(对象)时，可以为其指定不同的健康值计算函数。因此将操作和类分离。同时，如果提供修改函数指针成员的方法，每个对象还能使用不同的计算方法
...
pr->draw();         //调用Rectangle::draw，但是静态类型为Shape，所以缺省参数Shape::Red，而不是Rectangle::draw的green
pc->draw();         //调用Shape::draw，但是静态类型为Shape，动态类型为circle，所以缺省参数Shape::Red
```
...
* **编译器不会把子类对象转换为父类对象**
    ...
    private继承意味着只有实现部分被继承，借口部分应略去(d对象是根据b对象而得，无他意)
...
* 当Widget需要访问Timer的protected成员时。因为对象组合(复合)后只能访问public成员，而private继承后可以访问protected成员。
...
* 可以减小Widget和Timer的编译依赖。如果是private继承，在**定义**Widget的文件中势必需要引入#include"timer.h"。 但如果采用复合的方式，可以把WidgetTimer放到另一个文件中，在Widget中使用WidgetTimer\*并**声明**WidgetTimer即可
...
·非必要不要使用virtual bases ·若必须使用virtual base classes，尽可能避免在其中放置数据，这样就不用担心这些classes身上的初始化带来的奇怪的事情
...
* 对类而言，显式接口是由函数签名表征的(如在类型的对应头文件找到接口），运行时多态由虚函数实现
...
* 对模板而言，隐式接口是由表达式的合法性表征的，编译期多态由模板初始化和函数重载的解析实现
  ...
  void doProcessing(T& w){
      if(w.size() > 10 && w != someNastyWidget){
          ...
          temp.normalize();
          temp.swap(w);
  }//隐式接口：w需支持size normalize swap等接口
  ```
...
·从属名称：template内出现的名称相依于某个template参数（如下面的const_iterator，因为他相依于template参数C）
·嵌套从属名称：从属名称在class内呈嵌套状态（如下面的c::const_iterator)
...
typename相关规则在不同的编译器上有不同的实践。某些编译器接收的代码原本该有typename却遗漏了；原本不该有typename却出现了；还有少数编译器（通常是较旧版本）根本就拒绝typename。这意味typename和“嵌套从属名称”之间的互动，也会在移植性方面给你带来一些麻烦（可以用local的typedef + typename + 嵌套从属名称来简化）
...
因此，当base class被指定为MsgSender<CompanyZ>时，其内不包含sendClear方法，那么derived class的LoggingMsgSender的sendClearMsg方法就会调用不存在sendClear
...
```c++
LogginMsgSender<CompanZ> zMsgSender;
MsgInfo msgData;
...
zMsgSender.sendCleadrMsg(msgData); //无法编译
//编译器知道base class此时是个template的特化版本MsgSender<CompanZ>，他没有sendclear函数，无法调用sendCleadrMsg
```
...
* **对于非类型模板参数产生的代码膨胀，用函数参数或成员变量来替换模板参数即可消除冗余**
    ...
    private:                           //因此，只有一份invert(int size) 节省了两个相同的invert代码（模板具现化时）
    ...
    ```
    最后是父类如何访问矩阵数据。原本这些数据在派生类中，但是因为invert核心代码转移到了父类，那么父类必须有办法访问这些数据。可以在调用SquareBase::invert时把内存地址也一起告知父类，但如果矩阵类中有很多函数都需要这些信息就需要为每个函数添加一个这样的参数。因此，可以把**数据地址**直接放在父类中
    
    ```c++
    template<typename T>
    class SquareBase{
    protected: 
        SquareBase(size_t n, T* pMem) : size(n), pData(pMem)P{}
...
Top* pt1 = new Middle;  //middle隐式转换成Top*
smartPtr<Top> pt1 = smartPtr<Middle>(new Middle) //不能隐式转换
```
...
public:
    ...
    SmartPtr(const SmartPtr<U> &other)
     : heldPtr(other.get()) {...} //使得只有当存在U*隐式专换为T*才能通过编译
...
//此时oneHaf被声明为Rational<int> 也被具现化，而friend函数也会被自动声明出来，而且非函数模板，因此编译器可以使用隐式转换
```

**如果**上面只**有函数声明**（上面已经是可以连接的形式，因为是函数定义于类内了），而函数定义在类外，那么会报链接错误。当传入第一个参数oneHalt时，会具现化Rational\<int\>，编译器也就知道了我们要调用传入两个Rational\<int\>的版本，但是那个函数只在类中进行了声明，并没有定义，不能依赖类外的operator\* template提供定义，我们必须自己定义，所以会出现链接错误。解决方法就是像上面一样定义于类内

这样看起来有点像是member函数，但是因为friend关键字，所以实际是non-member函数，就可以实现所有实参进行隐式转换）。如果去掉friend关键字，就成了member函数，但是此时参数也只能有1个，就不能实现所有参数的隐式转换
...
//helper template //doMultipy定义在另外的头文件
template<typename T>
...
traits解法就是TMP，tpyeid比traits效率低，因为traits引发“编译器发生于类型身上的if...else计算”
...
```c++
widget* pw = new widegt;
```

假设第一个函数调用成功（new），第二个函数抛出异常（构造函数）就会有内存泄漏，因为没有获得指针无力归还指针。
...
Base* pb = new Base; //用的正常形式的operator new
Base* pb = new(std::cerr) Base; //用placement new
Derived* pd = new (std::clog) Derived; //错误，base placement new被掩盖了
Derived* pd = new Derived; //用的derived正常形式的operator new
```

一个比较好的方法是建立一个base class，内含所有正常形式new 和 delete：
...
如 智能指针smart pointers tr1::function tr1::bind 等
