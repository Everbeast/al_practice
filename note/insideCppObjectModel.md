...
第4种情况不展现“位逐次拷贝”是因为虚基类子对象部分能够正确的初始化。1）如果使用虚基类子类的一个对象，初始化虚基类子类的另一个对象，那么“位逐次拷贝”绰绰有余；（如Raccoon object来作为另一个Raccoon object的初值）2）但是如企图以一个虚基类子类的子类的对象，初始化一个虚基类子类的对象，编译器就必须判断“后续当程序员企图存取其虚基类子对象时能否正确执行”，因此必须合成一个拷贝构造函数，安插一些代码以设定虚基类指针和偏移量的初值（或只是简单地确定它没有被抹消）（如用RedPanda 的对象来座位Raccoon对象的初值）
...
```c++
Raccoon *ptr; //不知道指针是否真正的Raccon object
...
int main(){
    ...
    cout << "  A:" << sizeof(A) << endl;      //A:12，和机器，编译器相关
    return 0;
}
```
...
* **对于A**：类A的大小由下列因素决定
	...
	* **类Y的大小减去“因虚基类而配置的大小”** 4 byte
	* **类Z的大小减去“因虚基类而配置的大小”** 4 byte
	* **类A自己的大小**（这里是0 byte）
	* **对齐的要求** 9->12 byte
...
```c++
pv = (Vertex*)(((char*)&v3d) + sizeof(Point3d));//指针偏移到上图的Vertex *next处
```
...
bar()
{
    ...
    minval = min(val1, val2); //展开会 minval = val1 < val2 ? val1 : val2;
    minval = min(1024, 2048); //直接拥抱常量 minval = 1024
    minval = min( foo(), bar() + 1); //需要导入临时变量，避免重复求值
    /*
    int t1; int t2;
...
int t1;
int t2;
minval = ((__min_lv_minval_00 = val1 < va2 ? val1 : val2), __min_lv_minval_00) +
